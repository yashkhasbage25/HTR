...
class Network:
    ...
    class LeNet(nn.Module):
        def __init__(self, obj):
            ...
            dummy_out = self.dummy_forward(torch.zeros((10, obj.input_ch, obj.padded_im_size, obj.padded_im_size)))
    ...
    class BasicBlockDrop(nn.Module):
        ...
        def __init__(self, inplanes, planes, last, stride=1, downsample=None):
            super(Network.BasicBlockDrop, self).__init__()
            self.conv1 = Network.conv3x3(inplanes, planes, stride)
            self.bn1 = nn.BatchNorm2d(planes)
            self.relu1 = nn.ReLU(inplace=True)  
    ...
    class ResNetDrop(nn.Module):
        ...
        def __init__(self, obj, block, layers):
            self.obj = obj
            self.inplanes = 64
            super(Network.ResNetDrop, self).__init__()
    ...
    class ResNet18Drop(ResNet):
        def __init__(self, obj):
            super(Network.ResNet18Drop, self).__init__(obj, Network.BasicBlockDrop, [2, 2, 2, 2])
    ...
    class VGGDrop(nn.Module):
        def __init__(self, obj, net_type, batch_norm):
            super(Network.VGGDrop, self).__init__()
    ...
    class VGG11Drop(VGG):
        def __init__(self, obj):
            super(Network.VGG11Drop, self).__init__(obj, 'A', False)

    class VGG16_bnDrop(VGG):
        def __init__(self, obj):
            super(Network.VGG16_bnDrop, self).__init__(obj, 'D', True)