...
def parse_args():
    ...
    parser.add_argument('--middle', 
                        action='store_true',
                        help='penalize middle layers, otherwise penalize non-middle layers'
                        )
...
def get_penalization_mask(model, penalize_middle):

    layer_names = list(layer_name for (layer_name, _) in model.named_parameters())

    mask = np.zeros(len(layer_names))
    for i in range(len(mask) // 4, (len(mask) * 3) // 4 + 1):
        mask[i] = 1
    if not penalize_middle:
        mask = 1 - mask
    return mask.tolist()


def get_trace_loss(model, loss, hi, penalization_masl):

    niters = hi
    V = list()
    for _ in range(niters):
        V_i = [torch.randn_like(p, device=device) * m for (p, m) in zip(model.parameters(), penalization_mask)]
        # for V_ij in V_i:
        #     V_ij[V_ij == 0] = -1
        V.append(V_i)
    trace = list()
        ...
        with torch.enable_grad():
            for step, (batch, truth) in enumerate(dataloaders['train']):
                ...
                if step % args.freq == 0 and epoch >=args.sp:
                    trace_loss = get_trace_loss(model, loss, args.hi, penalization_mask)
...
if __name__ == '__main__':
    ...
    for rerun in range(args.reruns):
        ...
        penalization_mask = get_penalization_mask(model, args.middle)
