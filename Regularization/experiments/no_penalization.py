...
def parse_args():
    ...
    file_purpose = '''
    train a network with no penalization
    '''
    ...
    model_choices = ['VGG11_bn', 'ResNet18', 'DenseNet3_40', 'LeNet', 'VGG11', 'VGG19', 'VGG19_bn']
...
def train(model,
    ...
    logger = logging.getLogger('no_penalization')
...
if __name__ == '__main__':
    ...
    logging_file = osp.join(log_dir, 'no_penalization.log')
    logger = logging.getLogger('no_penalization')
    ...
    for rerun in range(args.reruns):
        ...
        if args.model in ['VGG11_bn', 'ResNet18', 'DenseNet3_40', 'LeNet', 'VGG11', 'VGG19', 'VGG19_bn']:
        ...
        optimizer = optim.SGD(model.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=0.0)
    ...
    stats_path = osp.join(ckpt_dir, 'no_penalization_train_stats.pkl')
    ...
    for phase in ['train', 'test']:
        print('phase:', phase)
        accs = np.array([t[phase] for t in train_test_stats])
        accs = accs[:, -1, :]
        print('shape', accs.shape)
        mean = accs.mean(axis=0)
        std = accs.std(axis=0)
        print('accs top@1: {:.3f} +- {:.3f}, top@5: {:.3f} +- {:.3f}'.format(mean[0], std[0], mean[1], std[1]))